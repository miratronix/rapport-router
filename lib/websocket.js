...
/**
 * Creates a response object for an HTTP message.
 *
 * @param {object} [wrappedSocket] The wrapped socket to use for responding.
 * @param {string} [requestId] The request ID to respond to.
 * @return {object} The resposne object with real operations if it's a request, and no-op operations otherwise.
 */
const createResponseObject = (wrappedSocket, requestId) => {
    let res;
    ...
    if (requestId) {
        res = {
            ...
            send: (body) => {
                const response = { _s: res._status, _b: body };
...
const handleHttpRequest = (wrappedSocket, router, requestId, request) => {
    router.handle(createRequestObject(request, requestId), createResponseObject(wrappedSocket, requestId));
};
 ...
 */
const handleHttpMessage = (router, msg) => {
    router.handle(createRequestObject(msg), createResponseObject());
};
...
module.exports = (wrappedSocket) => {
    ...
    wrappedSocket._functions.onMessage.handleMessage = (standardSocket, wrappedSocket, requestCache, options, msg, handler) => {
        if (isRoutable(options, msg)) {
            handleHttpMessage(options.router, msg);
        } else {
